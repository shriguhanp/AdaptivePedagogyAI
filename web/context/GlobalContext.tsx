...
const LANGUAGE_STORAGE_KEY = "adaptivex-language";
...
interface QuestionState {
  ...
  uploadMode: "kb" | "document"; // Source type for custom mode
  logs: LogEntry[];
...
interface GlobalContextType {
  ...
  // UI Settings
  uiSettings: { theme: "light" | "dark"; language: "en" };
  refreshSettings: () => Promise<void>;
  updateTheme: (theme: "light" | "dark") => Promise<void>;
  updateLanguage: (language: "en") => Promise<void>;
...
export function GlobalProvider({ children }: { children: React.ReactNode }) {
  ...
  const refreshSettings = async () => {
    ...
    const storedLanguage =
      typeof window !== "undefined"
        ? (localStorage.getItem(LANGUAGE_STORAGE_KEY) as "en") || "en"
        : "en";
  ...
  const updateLanguage = async (newLanguage: "en") => {
    // Update UI immediately
  ...
  const DEFAULT_NAV_ORDER: SidebarNavOrder = {
    start: ["/", "/history", "/knowledge", "/notebook", "/flashcard", "/quiz"],
    learnResearch: [
      "/question",
      "/solver",
      "/guide",
    ...
    const loadSidebarSettings = async () => {
      try {
        ...
        if (response.ok) {
          ...
          if (data.nav_order) {
            // Ensure flashcard is present (migration)
            const nav = data.nav_order;
            if (!nav.start.includes("/flashcard")) {
              nav.start.push("/flashcard");
            }
            setSidebarNavOrderState(nav);
          }
        }
      } catch (e) {
