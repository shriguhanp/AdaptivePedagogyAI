...
export const ResearchDashboard: React.FC<ResearchDashboardProps> = ({
  ...
  const planningContent = (
    <div className="flex-1 flex items-center justify-center">
      <div
        className={`bg-white dark:bg-slate-800 rounded-xl border shadow-sm p-8 flex flex-col items-center justify-center text-center max-w-2xl w-full ${isPlanningActive
          ? "border-blue-200 dark:border-blue-800"
          : "border-slate-200 dark:border-slate-700"
          }`}
      >
        <div
          className={`w-16 h-16 rounded-full flex items-center justify-center mb-4 ${isPlanningActive
  ...
  const reportingContent = (
    <div className="flex-1 flex items-center justify-center">
      <div
        className={`bg-white dark:bg-slate-800 rounded-xl border shadow-sm p-8 flex flex-col items-center justify-center text-center max-w-lg w-full ${isReportingActive
      ...
      >
        <div
          className={`w-16 h-16 rounded-full flex items-center justify-center mb-4 ${isReportingActive
          ...
          <div className="mt-6 w-full text-left bg-slate-50 dark:bg-slate-700 rounded-lg p-4 border border-slate-100 dark:border-slate-600 max-h-[200px] overflow-y-auto">
            ...
            <ul className="space-y-2">
              {reporting.outline.sections.map((section, i) => (
                ...
                >
                  <span
                    className={`w-5 h-5 rounded text-xs flex items-center justify-center font-medium shrink-0 ${isReportingActive &&
                      reporting.sectionIndex !== undefined &&
                  ...
                  <span
                    className={`line-clamp-1 ${isReportingActive &&
                      reporting.sectionIndex !== undefined &&
                      i + 1 === reporting.sectionIndex
    ...
    <div className="flex flex-col h-full bg-slate-50/50 dark:bg-slate-900/50">
      ...
      <div className="bg-white dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700 px-6 py-4 flex items-center justify-between shadow-sm shrink-0 z-20 relative">
        <div className="flex items-center gap-4">
          <div
            className={`p-2 rounded-lg ${global.stage === "idle"
        ...
        <div className="flex bg-slate-100 dark:bg-slate-700 p-1 rounded-lg border border-slate-200 dark:border-slate-600">
          <button
            ...
            className={`flex items-center gap-2 px-4 py-1.5 rounded-md text-sm font-medium transition-all ${activeView === "process"
          ...
          <button
            ...
            className={`flex items-center gap-2 px-4 py-1.5 rounded-md text-sm font-medium transition-all ${activeView === "report"
              ? "bg-white dark:bg-slate-600 text-indigo-600 dark:text-indigo-300 shadow-sm border border-slate-100 dark:border-slate-500"
              : !reporting.generatedReport && global.stage !== "completed"
                ? "text-slate-300 dark:text-slate-600 cursor-not-allowed"
                : "text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200"
        ...
        {isCompleted && onReset && (
          <button
            onClick={onReset}
            className="ml-4 flex items-center gap-2 px-4 py-2 bg-indigo-600 text-white rounded-lg text-sm font-medium hover:bg-indigo-700 transition-colors shadow-sm"
          >
