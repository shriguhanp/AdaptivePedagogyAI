...
export default function IdeaGenPage() {
  ...
  const handleNewTask = () => {
    setGeneratedIdeas([]);
    setUserThoughts("");
    clearAllSelections(); // Use existing function to clear selections
    setGenerationStatus("");
    setProgress(null);
    setIdeaToSave(null);
  };
  ...
  const handleBackToEdit = () => {
    ...
    setGenerationStatus("");
    setProgress(null);
    // Keep userThoughts and selectedRecords
  };
                              ...
                              <div className="space-y-1">
                                {records.map((record) => (
                                  <div
                                    ...
                                    className={`p-2 rounded-lg cursor-pointer transition-all border ${selectedRecords.has(record.id)
                                  ...
                                  >
                                    <div className="flex items-center gap-2">
                                      <div
                                        className={`w-4 h-4 rounded border flex items-center justify-center shrink-0 ${selectedRecords.has(record.id)
          ...
          {generatedIdeas.length > 0 && !isGenerating && (
            <div className="flex items-center gap-2 ml-auto">
              ...
              <button
                ...
                className="px-3 py-1.5 text-xs text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-lg transition-colors flex items-center gap-1 border border-slate-200 dark:border-slate-600"
                title={t("Clear results but keep inputs")}
              >
              ...
              <button
                ...
                className="px-3 py-1.5 text-xs text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-lg transition-colors flex items-center gap-1 border border-slate-200 dark:border-slate-600"
                title={t("Clear everything and start new")}
              >
          ...
          {generatedIdeas.length === 0 && !isGenerating ? (
            ...
            <div className="space-y-4">
              {generatedIdeas.map((idea) => (
                <div
                  key={idea.id}
                  className={`rounded-2xl border transition-all ${idea.selected
                ...
                >
                  ...
                  <div className="p-4 flex items-start gap-3">
                    ...
                    <button
                      ...
                      className={`w-6 h-6 rounded-full border-2 flex items-center justify-center shrink-0 transition-all ${idea.selected
                        ? "bg-amber-500 border-amber-500 text-white"
