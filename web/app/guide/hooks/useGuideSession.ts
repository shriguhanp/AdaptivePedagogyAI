...
export function useGuideSession() {
  ...
  const sendMessage = useCallback(
    async (message: string) => {
      ...
      console.log("[sendMessage] Current chat messages before:", chatMessages.length);

      addChatMessage("user", message, `user-${Date.now()}`);

      console.log("[sendMessage] Added user message to state");
      ...
      try {
        ...
        if (data.success) {
          ...
          console.log("[sendMessage] Added assistant message to state");
        } else {
          addChatMessage(
            "assistant",
  ...
  // Reset session to start a new one
