...
const STORAGE_PREFIX = "adaptivex_";
...
export function loadFromStorage<T>(key: string, defaultValue: T): T {
  ...
  try {
    ...
    // Migration logic: if new key not found, check old deeptutor_ prefix
    if (!raw) {
      const oldPrefixedKey = "deeptutor_" + key;
      raw = localStorage.getItem(oldPrefixedKey);

      if (raw) {
        console.info(`Migrating data for key: ${key} from deeptutor_ prefix to adaptivex_`);
        // Copy to new key
        localStorage.setItem(prefixedKey, raw);
        // Optionally: localStorage.removeItem(oldPrefixedKey); // Keep old for now just in case
      }
    }
...
/**
 * Clear all AdaptiveX data from localStorage
 */
export function clearAllStorage(): void {
  ...
  try {
    ...
    console.info(`Cleared ${keysToRemove.length} AdaptiveX storage items`);
  } catch (error) {
